...
class Conv2dModels:
    ...
    class BasicModel(BaseModel):
        ...
        def parse_shortcut_kwargs(cls, cmd: str, prefix: str,
            ...
            cmd = cmd.split('_')[1:]
            sf = cls.shortcut_prefix_fields(specifics,le,alow)
            args_tmp = { sf.get(i[:le],sf.get(i,i)) : i[le:] for i in cmd}
            args = {}
    ...
    class Conv2d(BasicModel):
        ...
        @classmethod
        def shortcut(cls,
            cmd: str = "conv_i3_o32_k3_s1_p1_d1_bs_bit", prefix: str = "conv",
            specifics:Dict[str,str]={'bias':'bs', 'bit':'bit',
                                    'padding_mode':None, 'scale_op':None}):
            kwargs = cls.parse_shortcut_kwargs(cmd,prefix,specifics)
