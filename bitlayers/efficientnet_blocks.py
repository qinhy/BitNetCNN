...
from . import create_conv2d, DropPath, make_divisible, create_act_layer, create_aa, to_2tuple, LayerType,\
...
class EfficientnetModels:
    class SqueezeExcite(Conv2dModels.SqueezeExcite):pass
    
class EfficientnetModules:
    class SqueezeExcite(Conv2dModules.SqueezeExcite):pass
...
class ConvBnAct(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = norm_act_layer(out_chs, inplace=True)
...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(
        ...
        # map timm-style pad_type to something nn.Conv2d accepts
        def _convert_padding(p):
            # handle typical string types from timm
            if p in ('same', 'SAME'):
                return 'same'
            if p in ('valid', 'VALID'):
                return 'valid'
            # empty string or None -> no extra padding
            if p in ('', None):
                return 0
            # if it's already an int/tuple, or some other valid value, pass through
            return p
        ...
        if s2d == 1:
            ...
            self.norm_s2d = norm_act_layer(sd_chs, sd_chs)
        ...
        self.norm1 = norm_act_layer(in_chs, inplace=True)
        ...
        self.conv_pw = nn.Conv2d(
            ...
            stride=1,
            padding=_convert_padding(pad_type),
            bias=False,
        )
        self.norm2 = norm_act_layer(out_chs, inplace=True, apply_act=self.has_pw_act)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = norm_act_layer(mid_chs, inplace=True)
        ...
        self.norm2 = norm_act_layer(mid_chs, inplace=True)
        ...
        self.norm3 = norm_act_layer(out_chs, apply_act=False)
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate else nn.Identity()
...
class EdgeResidual(nn.Module):
    ...
    def __init__(
        ...
        self.norm2 = norm_act_layer(out_chs, apply_act=False)
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate else nn.Identity()
