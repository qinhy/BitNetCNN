...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = Bit.Linear(dim, dim * 3, bias=qkv_bias)
        self.q_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.norm = norm_layer(dim) if scale_norm else nn.Identity()
        self.proj = Bit.Linear(dim, dim, bias=proj_bias)
...
class AttentionRope(nn.Module):
    ...
    def __init__(
        ...
        if qkv_fused:
            self.qkv = Bit.Linear(dim, attn_dim * 3, bias=qkv_bias)
            self.q_proj = self.k_proj = self.v_proj = None
        else:
            self.qkv = None
            self.q_proj = Bit.Linear(dim, attn_dim, bias=qkv_bias)
            self.k_proj = Bit.Linear(dim, attn_dim, bias=qkv_bias)
            self.v_proj = Bit.Linear(dim, attn_dim, bias=qkv_bias)

        self.q_norm = norm_layer(head_dim) if qk_norm else nn.Identity()
        self.k_norm = norm_layer(head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.norm = norm_layer(attn_dim) if scale_norm else nn.Identity()
        self.proj = Bit.Linear(attn_dim, dim, bias=proj_bias)
        self.proj_drop = nn.Dropout(proj_drop)
