...
def _create_fc(num_features, num_classes, use_conv=False):
    if num_classes <= 0:
        ...
        fc = Bit.Conv2d(num_features, num_classes, 1, bias=True)
    else:
        fc = Bit.Linear(num_features, num_classes, bias=True)
    return fc
...
class NormMlpClassifierHead(nn.Module):
    ...
    def __init__(
        ...
        linear_layer = partial(Bit.Conv2d, kernel_size=1) if self.use_conv else Bit.Linear
    ...
    def reset(self, num_classes: int, pool_type: Optional[str] = None):
        ...
        linear_layer = partial(Bit.Conv2d, kernel_size=1) if self.use_conv else Bit.Linear
        if self.hidden_size:
            if ((isinstance(self.pre_logits.fc, Bit.Conv2d) and not self.use_conv) or
                    (isinstance(self.pre_logits.fc, Bit.Linear) and self.use_conv)):
                with torch.no_grad():
...
class ClNormMlpClassifierHead(nn.Module):
    ...
    def __init__(
        ...
        if hidden_size:
            self.pre_logits = nn.Sequential(OrderedDict([
                ('fc', Bit.Linear(in_features, hidden_size)),
                ('act', act_layer()),
            ]))
            self.num_features = hidden_size
        else:
            self.pre_logits = nn.Identity()
        self.drop = nn.Dropout(drop_rate)
        self.fc = Bit.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
