...
class NonLocalAttn(nn.Module):
    ...
    def __init__(self, in_channels, use_scale=True,  rd_ratio=1/8, rd_channels=None, rd_divisor=8, **kwargs):
        ...
        self.t = Bit.Conv2d(in_channels, rd_channels, kernel_size=1, stride=1, bias=True)
        self.p = Bit.Conv2d(in_channels, rd_channels, kernel_size=1, stride=1, bias=True)
        self.g = Bit.Conv2d(in_channels, rd_channels, kernel_size=1, stride=1, bias=True)
        self.z = Bit.Conv2d(rd_channels, in_channels, kernel_size=1, stride=1, bias=True)
...
class BilinearAttnTransform(nn.Module):
    ...
    def __init__(self, in_channels, block_size, groups, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d):
        ...
        self.conv_p = Bit.Conv2d(groups, block_size * block_size * groups, kernel_size=(block_size, 1))
        self.conv_q = Bit.Conv2d(groups, block_size * block_size * groups, kernel_size=(1, block_size))
