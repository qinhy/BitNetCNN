...
class SEModule(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = Bit.Conv2d(channels, rd_channels, kernel_size=1, bias=bias)
        self.bn = norm_layer(rd_channels) if norm_layer else nn.Identity()
        self.act = create_act_layer(act_layer, inplace=True)
        self.fc2 = Bit.Conv2d(rd_channels, channels, kernel_size=1, bias=bias)
...
class EffectiveSEModule(nn.Module):
    ...
    def __init__(self, channels, add_maxpool=False, gate_layer='hard_sigmoid', **_):
        ...
        self.add_maxpool = add_maxpool
        self.fc = Bit.Conv2d(channels, channels, kernel_size=1, padding=0)
        self.gate = create_act_layer(gate_layer)
...
class SqueezeExciteCl(nn.Module):
    ...
    def __init__(
        ...
        if not rd_channels:
            rd_channels = make_divisible(channels * rd_ratio, rd_divisor, round_limit=0.)
        self.fc1 = Bit.Linear(channels, rd_channels, bias=bias)
        self.act = create_act_layer(act_layer, inplace=True)
        self.fc2 = Bit.Linear(rd_channels, channels, bias=bias)
        self.gate = create_act_layer(gate_layer)
