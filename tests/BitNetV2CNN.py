...
from bitlayersV2 import bit, convs, acts, norms, nn

# from bitlayersV2.acts import ActModels
# from bitlayersV2.norms import NormModels
# ----------------------------
# Simple BitNet block & model
# ----------------------------
InvertedResidual = convs.Convs.InvertedResidual
InvertedResidualModule = convs.Convs.InvertedResidual
...
class NetCNN(nn.Module):
    ...
    stem:convs.Convs.Conv2dNormAct = None
    ...
    def model_post_init(self, __context):
        ...
        self.drop2d_p = self.drop_p/2
        in_channels = self.in_channels
        num_classes = self.num_classes
        expand_ratio = self.expand_ratio
        drop_p = self.drop_p
        bias = self.bias
        def act(): return acts.Acts.SiLU
        def norm(): return norms.Norms.BatchNorm2d

        self.stem:convs.Convs.Conv2dNormAct = convs.Convs.Conv2dNormAct(
        ...
        def cconvs():
            return dict(conv_pw_exp_layer=convs.Convs.Conv2dPointwiseNormAct(
                            in_channels=-1,norm=norm(),act=act(),device=self.device),
                        conv_dw_layer=convs.Convs.Conv2dDepthwiseNormAct(
                            in_channels=-1,norm=norm(),act=act(),device=self.device),
                        conv_pw_layer=convs.Convs.Conv2dPointwiseNorm(
                            in_channels=-1,norm=norm(),device=self.device),
                        drop_path_rate=self.drop2d_p,
                        padding=0,exp_ratio=2.0,stride=2,bias=bias,
                        device=self.device)
...
def main():
    ...
    trainer.fit(lit, datamodule=dm.build(), val_first=True)
