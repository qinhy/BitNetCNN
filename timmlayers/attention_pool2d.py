...
class RotAttentionPool2d(nn.Module):
    ...
    def __init__(
        ...
        if qkv_separate:
            self.q = Bit.Linear(in_features, embed_dim, bias=qkv_bias)
            self.k = Bit.Linear(in_features, embed_dim, bias=qkv_bias)
            self.v = Bit.Linear(in_features, embed_dim, bias=qkv_bias)
            self.qkv = None
        else:
            self.qkv = Bit.Linear(in_features, embed_dim * 3, bias=qkv_bias)
        self.drop = nn.Dropout(drop_rate)
        self.proj = Bit.Linear(embed_dim, self.out_features)
    ...
    def reset(self, num_classes: Optional[int] = None, pool_type: Optional[str] = None):
        ...
        if num_classes is not None:
            self.proj = Bit.Linear(self.in_features, num_classes) if num_classes > 0 else nn.Identity()
